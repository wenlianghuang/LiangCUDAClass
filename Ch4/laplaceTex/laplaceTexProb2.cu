...
__align__(8) texture<float> texOld;
...
int main(void)
{
    ...
    //button
    for(int x=0; x<Nx; x++) {
      h_new[x+Nx*(Ny-1)]=5.0f;  
      h_old[x+Nx*(Ny-1)]=5.0f;
    }
    ...
    //right
    for(int y = 1; y<=Ny; y++) {
      h_new[Nx-1 + Nx*(y-1)] = -2.0f;
      h_old[Nx-1 + Nx*(y-1)] = -2.0f;
    }

    FILE *out1;                 // save initial configuration in phi_initialProb2.dat
    out1 = fopen("phi_initialProb2.dat","w");
...
//    }
    ...
    if(CPU>0) {
      ...
      cudaBindTexture(NULL, texOld, d_old, size);
...
//        printf("iteration = %d\n",iter);
      ...
      outg = fopen("phi_GPU_TexProb2.dat","w");
...
//        printf("iteration = %d\n",iter);
      ...
      outc = fopen("phi_CPU_TexProb2.dat","w");
