	  ...
	  float* phiT_old, float* phi0_new, float* C,float omega)
{
    ...
    if (skip == 0) {
      phi0_new[site] = 0.25*omega*(b+l+r+t) + (1.0 - omega)*c;
...
int main(void)
{
 ...
 //  Get the input parameters.
    ...
    printf("  Solve Heat Diffusion equation on 2D lattice with boundary conditions\n");
 ...
 //  Allocate field vector h_phi in host memory
    ...
    //Top
    for (int x=0; x<Nx; x++) {
      h_1[x+Nx*(Ny-1)]=373.0; //100 celsius
      h_2[x+Nx*(Ny-1)]=373.0; //100 celsius
    }  
...
//  Compute the CPU solution.
	...
	while ( (error > eps) && (iter < MAX) ) {
	    ...
	    for (int x=0; x<Nx; x++) { 
		    ...
		    r = h_old[xp1]; 
		    t = h_old[yp1]; 
		    h_new[site] = 0.25*omega*(b+l+r+t) + (1.0-omega)*h_old[site];
