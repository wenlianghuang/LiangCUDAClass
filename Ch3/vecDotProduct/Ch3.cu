...
//
// (for GTX970)
// nvcc -arch=compute_52 -code=sm_52,sm_52 -O2 -m64 -o  Ch3 Ch3.cu
...
//    printf("The number of blocks per grid:%d\n",blocksPerGrid);
    ...
    VecDot <<< blocksPerGrid, threadsPerBlock, sm >>>(d_A, d_B, d_C, N);//The third paremeter in <<<>>> type is "size_t", but actually is not necssary
