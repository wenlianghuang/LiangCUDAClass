...
__global__ void matAdd(float *matrixA, float *matrixB, float *matrixC, int matSize)
{
...
void printMatrix(float *matrix, int size, char * matrixName)
{
...
bool checkIfMatricesEqual(float * mat1, float * mat2, int matSize)
{
    int i = 0;
    for( ; i < matSize; i++)
       if(mat1[i] != mat2[i]){
...
int main()
{
   ...
   float *matA, *matB, *matC, *matCFromGPU;
   float *gpuMatA, *gpuMatB, *gpuMatC;            //matrices for Device
   ...
   matA = (float *)malloc(matrixSize * sizeof(int) * matrixSize);
   matB = (float *)malloc(matrixSize * sizeof(int) * matrixSize);
   matC = (float *)malloc(matrixSize * sizeof(int) * matrixSize);
   ...
   **/
   ...
   while(!done)   
   {
       matCFromGPU = (float *)malloc(matrixSize * sizeof(int) * matrixSize);
